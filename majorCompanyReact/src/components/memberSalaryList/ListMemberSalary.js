...
import MemberSalaryCSS from './ListMemberSalary.module.css';

import {
  callGetSalaryManageAPI,
  callSalaryRequestAPI
...
function ListMemberSalary() {
  ...
  /* 지난 달 근태 확인용 연월 계산 */
  let yearAndMonth = ((day.getFullYear().toString()) + (day.getMonth().toString()));
  if(day.getMonth().toString() == "0") {
    yearAndMonth = ((day.getFullYear()  - 1).toString() + 12);
  } else if(parseInt(day.getMonth().toString()) < 10) { 
    yearAndMonth = ((day.getFullYear().toString()) + 0 + (day.getMonth().toString()));
  }
  ...
  // console.log("whoIsTheMember", theMember);
  ...
  const attThisMonth = attList && attList.map((list) => list.attendanceDate.substr(0, 6));
  // console.log("attThisMonth: ", attThisMonth);
  ...
  for(let i = 0; i < (attList && attList.length); i++){
    ...
    if((attThisMonth && attThisMonth[i]) === yearAndMonth) { 
    ...
    if(attList && attList[i].attendanceOvertime === null) { 
      overTime[i] = 0;
    } else if(((attThisMonth && attThisMonth[i]) === yearAndMonth) && (attList && attList[i].attendanceOvertime !== null)) { 
  ...
  const onClickRequestSalaryHandler = () => {
      ...
      console.log('[SalaryRegistration] onClickRequestSalary');

      dispatch(callSalaryRequestAPI({	// 사원 급여 정보 조회
          form: form
      }));        
      
      alert('급여 지급 요청이 완료되었습니다.');  
  ...
  return (
    <>
      <div 
            className={ MemberSalaryCSS.backgroundDiv}
            style={ { backgroundColor: 'white'} }>
        
        {theMember && <div className={ MemberSalaryCSS.registerForm }>
